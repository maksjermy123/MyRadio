...
- Не читал сегодня — матрица взаимоисключающих состояний (первое подходящее):
  1. отстаёт по графику → «⚠️ Отстаёшь на N дней… продолжай с дня K» (N и K —
     тем же алгоритмом, что и в мини-аппе; K = позиция чтения, совпадает с тем,
     куда ведёт кнопка «Открыть план»);
  2. впереди графика (чтение наперёд, `max(days_done) > calendar_day`) →
     «🎯 Впереди графика на N дней», стрик добавляется только если серия жива.
     Раньше такие пользователи при протухшем стрике получали бессмысленное
     «Начни сегодня!» при 10+ прочитанных днях;
  3. на графике, стрик жив → «🔥 N дней подряд»;
  4. на графике, стрик протух, но дни уже отмечались → «Серия прервалась —
     сегодня хороший день её продолжить» (НЕ «Начни сегодня!» — это врало бы:
     человек уже начал; «Начни сегодня!» остаётся только в онбординге, режим 2).

**Эффективный стрик** (`_effective_streak`): напоминания и публичный срез
виджета «План» считают стрик действительным, только если `last_read_date`
∈ {сегодня, вчера} по МСК — тот же критерий, что и бейдж мини-аппа
(`effStreak`). Раньше у давно не открывавшего бота уведомление и виджет
показывали «🔥 5 дней подряд» при давно сломанной серии, расходясь с
мини-аппом. `max_streak` (личный рекорд) не «протухает» никогда.
