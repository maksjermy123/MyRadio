...
# Кнопка канала убрана из приветствия (см. bible_welcome_buttons) — константа
# пока оставлена на случай возврата кнопки, сейчас нигде не используется.
...
def bible_welcome_buttons() -> dict:
    ...
    Раньше здесь была вторая кнопка — ссылка на канал (CHANNEL_BTN_TEXT →
    CHANNEL_LINK) — убрали совсем: у приветствия, как и у ежедневного
    напоминания, должно быть ровно одно чёткое действие — «Открыть план
    чтения», без конкурирующего второго CTA."""
    return {"inline_keyboard": [[
        {"text": "📖 Открыть план чтения", "web_app": {"url": _bible_url()}},
    ]]}

def bible_start_keyboard() -> dict:
    """Постоянная клавиатура внизу чата — всегда только «Поделиться с другом».

    Раньше для НОВОГО пользователя здесь была ещё и кнопка «Старт» («единственный
    способ создать чат с ботом»). Но фолбэк-обработчик вебхука отвечает на ЛЮБОЕ
    первое сообщение — и в этом же обработчике вызывает sb_account_mark_started,
    т.е. к моменту установки клавиатуры чат уже создан и started_at проставлен.
    Кнопка «Старт» в ответ на само нажатие «Старт» — тавтология: из-за того, что
    флаг registered вычислялся ДО mark_started, при первом контакте он был
    всегда False, и кнопка висела у пользователя до ВТОРОГО нажатия — только
    второй «Старт» убирал её (реальный баг, подтверждённый вживую). Теперь
    клавиатура никогда не содержит «Старт»: для самого первого контакта есть
    нативная кнопка Telegram «START», а любое повторное сообщение всё равно
    попадает в тот же фолбэк-обработчик."""
    return {
        "keyboard": [[{"text": SHARE_BTN_TEXT}]],
...
def bible_reminder_button(plan_id: str) -> dict:
    ...
    канала осталось только на экране завершения плана в мини-аппе:
    кнопку канала убрали и из приветствия (см. bible_welcome_buttons)."""
    return {"inline_keyboard": [[
...
async def bible_webhook(request: Request):
    ...
    #   1) приветствие с одной инлайн-кнопкой, прикреплённой именно к нему
    #      (bible_welcome_buttons) — «Открыть план чтения»;
    #   2) короткая реплика, которая (пере)устанавливает постоянную
    #      клавиатуру внизу — только «Поделиться с другом», БЕЗ «Старт»:
    #      этот обработчик сам вызывает sb_account_mark_started, т.е. к
    #      моменту установки клавиатуры пользователь уже зарегистрирован,
    #      и предлагать «Старт» в ответ на нажатый «Старт» — тавтология
    #      (раньше из-за registered, вычисленного ДО mark_started, кнопка
    #      «Старт» пропадала только после ВТОРОГО нажатия).
